  ...
  <section class="blog-post" v-if="blog">
    ...
    <div class="related-blogs" v-if="relatedBlogs.length" data-aos="fade-up" data-aos-delay="500">
      ...
      <div class="related-blogs-container">
        <div 
          v-for="relatedBlog in filteredRelatedBlogs" 
          :key="relatedBlog.id" 
          class="related-blog-item"
        >
          <router-link :to="`/blogs/${relatedBlog.id}`">
            <img :src="getBlogPath(relatedBlog.image)" :alt="relatedBlog.title" class="related-blog-image" />
  ...
  computed: {
    ...
    filteredRelatedBlogs() {
      return this.relatedBlogs.filter((blog) => 
        blog.category === this.blog.category || 
